...
def train_vae(
    ...
    zs = np.random.uniform(
        -0.3, 0.3, [4, n_code]).astype(np.float32)
    zs = utils.make_latent_manifold(zs, 4)
    ...
    for idx in range(0, 4):
        ...
        train_xs = np.append(train_xs, temp_xs/255.0, axis=0)
        train_ts = np.append(train_ts, temp_ts/255.0, axis=0)
        train_ys = np.append(train_ys, temp_ys, axis=0)
    
    utils.montage(
            train_xs[:n_examples**2],
            output_path + '/input_train.png')
    utils.montage(
            train_ts[:n_examples**2],
            output_path + '/target_train.png')
    ...
    if files_valid != '':
        ...
        valid_ts /= 255.0
        utils.montage(
                valid_xs[:n_examples**2],
                output_path + '/input_valid.png')
        utils.montage(
                valid_ts[:n_examples**2],
    ...
    try:
        while not coord.should_stop() and epoch_i < n_epochs:
            ...
            if batch_i % img_step == 1:
                ...
                recon = sess.run(
                    ae['y'], feed_dict={ae['x']: train_xs[:n_examples**2],
                                        ae['train']: False,
                                        ae['keep_prob']: 1.0})
                # utils.montage(
                #    recon.reshape([-1] + crop_shape + [output_shape[-1]]),
                ...
                if files_valid != '':
                    ...
                    # Plot example reconstructions
                    recon = sess.run(
                        ae['y'], feed_dict={ae['x']: valid_xs[:n_examples**2],
                                            ae['train']: False,
                                            ae['keep_prob']: 1.0})
                    # utils.montage(
                    #    recon.reshape([-1] + crop_shape + [output_shape[-1]]),
                    ...
                    for i in range(0, round_valid):
                        ...
                        z_viz = np.append(
                            ...
                            sess.run([ae['z']], feed_dict={
                                ...
                                ae['keep_prob']: 1.0})[0])
                        label_viz = np.append(
                                label_viz, batch_ys[:, 0]).astype(int)
                        valid_i += 1
                    ...
                    # PCA
                    feat_viz, V = pca(z_viz, dim_remain=3)
                    nebula_viz = np.matmul(nebula3d, V)
                    # t-SNE
                    # feat_viz = TSNE(n_components=3).fit_transform(z_viz)
                    fig = plt.figure()
